                ...
                let authConfig : Raft.RESTlerTypes.Engine.RefreshableTokenOptions =
                    {
                        ...
                        refreshCommand =
                            ...
                            | Raft.Job.Authentication.TokenRefresh.MSAL secret ->
                                (sprintf "dotnet /raft-utils/auth/dotnet-core-3.1/AzureAuth.dll msal --secret \"%s\" --prepend-line \"{u'user1':{}}\"" secret)

                            | Raft.Job.Authentication.TokenRefresh.TxtToken secret ->
                                (sprintf "dotnet /raft-utils/auth/dotnet-core-3.1/AzureAuth.dll token --secret \"%s\" --prepend-line \"{u'user1':{}}\"" secret)
                    }
