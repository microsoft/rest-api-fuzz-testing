...
module ContainerInstances =
    ...
    let isJobRunFinished (g: IContainerGroup) =
        if Tags.isIdling g.Tags then
            false
        else if Tags.isGcReady g.Tags then
    ...
    let executePostRun (logger: ILogger) (cg: IContainerGroup) =
        async {
            let logInfo format = Printf.kprintf logger.LogInformation format
            let logError format = Printf.kprintf logger.LogError format
            match Tags.getPostRunTimeoutUTC cg.Tags with
            | Some t when t < System.DateTime.UtcNow ->
            ...
            | None ->
                ...
                | Some d ->
                    ...
                    //now wait post run duration before deleting the job
                    let! _ = cg.Update().WithTag(Tags.PostRunTimeoutUtc , sprintf "%A" (System.DateTime.UtcNow + d)).ApplyAsync().ToAsync
                    return false
        }
                                ...
                                let! postRunFinished =
                                    async {
                                        if jobRunFinished || isExpired then
                                            let! pr = executePostRun logger g
                                            if pr then
                                                do! stopJob g
                                            return pr
                                        else
